...
function bindGameMenuEvt(){
    $('#btn-game-play-now').click(function(e){
        e.preventDefault("$('#btn-game-play-now').click");
        console.log();
        OME.trackPush(dataLayer, OME.trackCopy.category, OME.trackCopy.playFeedHenry_play_action, OME.trackCopy.playFeedHenry_play_label, 'userAction');
...
function bindLeaderboardEvt(){
    ...
    $('#btn-game-leaderboard-share').click(function(e){ 
        ...
        FB.ui({
          ...
          picture: OME.baseUrl + '/img/share/share.default.facebook.png',
...
function bindGameEndEvt(){
    ...
    $('#btn-game-end-play-again').click(function(e){
        e.preventDefault();        
        OME.trackPush(dataLayer, OME.trackCopy.category, OME.trackCopy.playFeedHenry_playAgain_action, OME.trackCopy.playFeedHenry_playAgain_label, 'userAction');
        resetGameLevel();
    });
    ...
    $('#btn-game-end-fb-connect').click(function(e){
        e.preventDefault();
        OME.trackPush(dataLayer, OME.trackCopy.category, OME.trackCopy.playFeedHenry_fbConnect_action, OME.trackCopy.playFeedHenry_fbConnect_label, 'userAction');
            ...
            if (response.status === 'connected') {            
                ...
                $.ajax({
                    ...
                    data: { 
                        ...
                        key: key,
                        hash: sha1(gameObj.totalSec + key + OME.sk)
                    }
                }).done(function(msg) {
...
function showMap(toShow){    
    if(toShow){
        ...
        if(gameObj.questions.length == 0){ return; }
...
function resetGameLevel(){
    ...
    if(!gameObj.loaded){
        console.log('gameObj not loaded');
        showGamePreloader(true);
        preloadGameItems(gameObj.items, 0, function(){ gameItemsLoaded(); });
    }else{
        setupCurrentLevel();
    ...
    console.log('done resetGameLevel');
}
...
function showPitStop(toShow){
    //update henry sprite
    for(var i=1; i<=14; i++){
        $('.henry-sprite').removeClass('stage'+i);
    }
    $('.henry-sprite').addClass('stage'+(gameObj.curLevel-1));
...
function showGameCompleted(toShow){
    ...
    if(toShow){
        OME.trackPush(dataLayer, OME.trackCopy.category, OME.trackCopy.playFeedHenry_gameComplete_action, OME.trackCopy.playFeedHenry_gameComplete_label, 'userAction');
...
function useHint(){
    ...
    //random
    var answers = gameObj.questions[gameObj.curLevel-1].answer.replace(/\s/g, '').split(''),
        inputs = [],
        i, j, index;

    $('.answer .answer-slot').each(function(){
    ...
    var randStartIndex = Math.floor(Math.random()*length);
    for(i=0; i<inputs.length; i++){
        j = (i + randStartIndex) % inputs.length;
        if(inputs[j] != answers[j]){
            index = j;
            break;
        }
    }    
    ...
    console.log("Not: " + inputs[index]);
    console.log("Is: " + answers[index]);

    if(inputs[index] == ""){
        console.log("hint on empty slot");

        //remove the correct answer from selection box
        $('.answer-sel[data-char="'+answers[index]+'"]:first').addClass('hide');
        console.log("took correct selection: " + answers[index]);

        //place the correct answer into the answer box  
        $('.answer-slot:nth('+index+')').removeClass('empty').append(
            $('.answer-sel[data-char="'+answers[index]+'"]:first').clone().removeClass('hide answer-sel').addClass('answer-selected fixed')
        );
            
        console.log("replace correct selection: " + answers[index]);     

    }else{
        console.log("hint on non-empty slot");

        //put back the wrong answer to selection box
        $('.answer-sel[data-char="'+inputs[index]+'"]:first').removeClass('hide');
        console.log("put back wrong selection: " + inputs[index]);
        ...
        //place the correct answer into the answer box
        console.log($('.answer-sel[data-char="'+answers[index]+'"]:first').clone());        

        $('.answer-slot:nth('+index+')').find('a').remove();
        $('.answer-slot:nth('+index+')').append(
            $('.answer-sel[data-char="'+answers[index]+'"]:first').clone().removeClass('hide answer-sel').addClass('answer-selected fixed')
        );
